...
export const UpdateForm6 = ({ handleChange, formData, setFormData }) => {
  ...
  useEffect(() => {
    if (Object.keys(formData).length > 0) {
      ...
      type.forEach((item, index) => {
        const name = nursery.find((nursery) => nursery.id === item)?.name || "";
        updatedFormData[`nurseryname${index + 1}`] = name;
      });

      const type2 = initialAdditionalFields2.map(
        (item) => item.causeofdeforestiontype
      );
  ...
  const addField = () => {
    ...
    setAdditionalFields([
      ...additionalFields,
      { id: highestId + 1, nurserytype: "", numberofnursery: "" },
    ]);
  };
  const removeField = (id) => {
    ...
    for (let key in updatedFormData) {
      if (key.startsWith("nurserytype") && key !== `nurserytype${id + 1}`) {
        ...
        key.startsWith("nurseryname") &&
        key !== `nurseryname${id + 1}`
      ) {
  ...
  const removeField2 = (id) => {
    ...
    for (let key in updatedFormData) {
      if (
        key.startsWith("causeofdeforestiontype") &&
        key !== `causeofdeforestiontype${id + 1}`
      ) {
        newFormData[`causeofdeforestiontype${causeofdeforestiontypeIndex}`] =
          updatedFormData[key];
        causeofdeforestiontypeIndex++;
      } else if (
        key.startsWith("causeofdeforestionname") &&
        key !== `causeofdeforestionname${id + 1}`
      ) {
        newFormData[`causeofdeforestionname${causeofdeforestionnameIndex}`] =
          updatedFormData[key];
          ...
          <React.Fragment key={field.id}>
            <div className="flex flex-wrap">
              <FormField
                ...
                value={
                  formData[`nurseryname${index + 1}`] ||
                  formData[`nurserytype${index + 1}`]
                }
                onChange={(option) => {
                  handleChanges({
                    target: {
        ...
        {additionalFields2.map((field, index) => (
          <React.Fragment key={field.id}>
            <FormField
              ...
              handleChange={handleChanges}
              value={
                formData[`causeofdeforestionname${index + 1}`] ||
                formData[`causeofdeforestiontype${index + 1}`]
              }
              onChange={(option) => {
                handleChanges({
                  target: {
