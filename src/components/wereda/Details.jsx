...
import { useDeleteWeredaByIdMutation, useGetWeredaByIdQuery, useGetWoredaQuery } from "../../redux/wereda/WeredaApiSlice";
...
export const WeredaDetails = () => {
  ...
  const { data: weredadata, isSuccess, isFetching ,error} = useGetWeredaByIdQuery(id);
  const { data: KebeleData, isSuccess: kebeleFetched } =
    useGetKebeleByWeredaQuery({id: id});
    const { data: wereda, isLoading, isSuccess:weredaSuccess } = useGetWoredaQuery({all: true});
  ...
  if (weredadata ||weredadata.woreda_resource || weredadata.woreda_institution) {
      ...
      <div>
        <div className="flex justify-between p-10">
          ...
          <div className="flex gap-4">
            <button className=" p-2 rounded-md text-sm bg-deletecolor hover:bg-customDark text-white font-semibold" onClick={() => deleteWereda(id)}>
